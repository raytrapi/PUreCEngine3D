...
uniform int numeroShadowMaps=0;

/*uniform vec3 posicionLuz;
uniform vec3 colorLuz;
uniform float watiosLuz;/**/

//layout (std140, binding = 2) uniform ConTextura {
//    bool conTextura[20];
//};
uniform bool conTextura; //=false
uniform sampler2D albedo_texture; // Color
uniform sampler2D normal_texture; // Dirección de rebote de la luz
uniform sampler2D diffuse_texture; // Iluminación
uniform sampler2D metalic_texture; // Metalico
uniform sampler2D displacement_texture; // Desperfectos
uniform sampler2D ambient_oclusion_texture; // AO

uniform sampler2D shadowMap; //No más de 20 luces

out vec4 frag_colour;

float ShadowCalculation(vec4 fragPosLightSpace, Light light, int idShadow){
...
void main() {
   ...
   float shadow=0;
   //float shadow = ShadowCalculation(lights[0].shadowMatrix* vec4(fragPos, 1.0),idText);
   //float shadow = ShadowCalculation( lights[0].shadowMatrix* vec4(fragPos, 1.0), lights[0],idText);
